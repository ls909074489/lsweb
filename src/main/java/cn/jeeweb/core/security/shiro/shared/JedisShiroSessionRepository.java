package cn.jeeweb.core.security.shiro.shared;
...
import cn.jeeweb.core.security.shiro.session.SessionStatus;
...
public class JedisShiroSessionRepository implements ShiroSessionRepository {
    public static final String REDIS_SHIRO_SESSION = "ls-shiro-session:";
    //这里有个小BUG，因为Redis使用序列化后，Key反序列化回来发现前面有一段乱码，解决的办法是存储缓存不序列化
    public static final String REDIS_SHIRO_ALL = "*ls-shiro-session:*";
...
//            byte[] key = SerializeUtil.serialize(buildRedisSessionKey(session.getId()));
        	byte[] key = SerializeUtil.toByteKey(buildRedisSessionKey(session.getId()));
...
//            byte[] value = SerializeUtil.serialize(session);
            byte[] value = SerializeUtil.toByteArray(session);
...
//            getJedisManager().deleteByKey(DB_INDEX,SerializeUtil.serialize(buildRedisSessionKey(id)));
        	getJedisManager().deleteByKey(DB_INDEX,SerializeUtil.toByteKey(buildRedisSessionKey(id)));
        } catch (Exception e) {
...
//            byte[] value = getJedisManager().getValueByKey(DB_INDEX, SerializeUtil.serialize(buildRedisSessionKey(id)));
            byte[] value = getJedisManager().getValueByKey(DB_INDEX, SerializeUtil.toByteKey(buildRedisSessionKey(id)));
//            session = SerializeUtil.deserialize(value, Session.class);
            session = (Session) SerializeUtil.toObject(value);
        } catch (Exception e) {
